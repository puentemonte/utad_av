...
extern double totalTime, currentFrames, currentLogicTime;
...
void RenderScene()
{
	...
	// Text
	char FPSStr[100];
	snprintf(FPSStr, 100, "FPS:%f", 1 / currentFrames);
	FONT_DrawString(vec2(0, SCR_HEIGHT - 16), FPSStr);

	char currentTimeString[100];
	snprintf(currentTimeString, 100, "TOTAL TIME:%f", totalTime);
	FONT_DrawString(vec2(0, SCR_HEIGHT - 32), currentTimeString);

	char tickString[100];
	snprintf(tickString, 100, "LOGIC TIME:%f", currentLogicTime);
	FONT_DrawString(vec2(0, SCR_HEIGHT - 48), tickString);
